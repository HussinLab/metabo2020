...
data <- list.files(path = "anova_inputs/timepoints_analysis/withCorrections/first_part", full.names = TRUE, pattern="*.csv")
data2 <- list.files(path = "anova_inputs/timepoints_analysis/withCorrections/second_part", full.names = TRUE, pattern="*.csv")

table_time = NULL;
for (i in 1:150){
    f <- read.table(file=data[i], sep="\t", header=T)
    f$subject_BL_DC <- factor(f$subject_BL_DC)
    f$age <- rescale(f$age, to = c(0,1))
    f$pairing <- rescale(f$pairing, to = c(0,1))
    f_long <- gather(f, BL_DC, value, BL_value:DC_value)
    lmer_model <- lmer(value ~ BL_DC+sex+age+diabetes+dyslipidemia + (1|pairing), data=f_long)
    summary <- summary(lmer_model)
    line = c(metabos[i],summary$coefficients[2], summary$coefficients[26])
    #metabo_name, log2FC state, pval state
...
for (i in 1:130){
    ...
    lmer_model <- lmer(value ~ BL_DC+sex+age+diabetes+dyslipidemia + (1|pairing), data=f_long)
    summary <- summary(lmer_model)
    line2 = c(metabos2[i],summary$coefficients[2], summary$coefficients[26])
    #metabo_name, log2FC state, pval state
    table_time = rbind(table_time,line2)
}

table_time <- as.data.frame(table_time)
table_time <- plyr::rename(table_time, c("V1"="Metabolites", "V2"="log2_FC", "V3"="pval"))
rownames(table_time) <- table_time$Metabolites
table_time$Metabolites <- NULL

table_time$log2_FC<-as.numeric(levels(table_time$log2_FC))[table_time$log2_FC]
table_time$pval<-as.numeric(levels(table_time$pval))[table_time$pval]

table_time$fdr <-p.adjust(table_time$pval,"fdr")


write.table(table_time, file="output_lmer_normalized.csv", sep="\t")
